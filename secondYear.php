...
// SQL query to fetch Second Year data with a "Status" column
$sql = "
  SELECT
    *,
    IF(
...
if ($result === false) {
    echo "Error fetching Second Year data: " . $connection->error;
...
<div class="container"> <!-- Container with rounded corners -->
    ...
    <h2>Second Year Checklist of Courses</h2>
    ...
    <div id="second-year-checklist">
        ...
        <?php else: ?>
            <p><b>No matching second year records found.</p>
        <?php endif; ?>
    </div>
...
function applySearch() {
    ...
    var rows = document.querySelectorAll("#second-year-checklist table tr"); // Get all rows
    ...
    var secondYearChecklist = document.getElementById("second-year-checklist");

    if (visibleCount === 0) {
        // If no rows are visible, replace the content with a message
        secondYearChecklist.innerHTML = "<p><b>No matching secondyear records found.</b></p>";
    } else {
        ...
        secondYearChecklist.querySelector("table").innerHTML += summaryRow; // Append the summary row
    }
}
