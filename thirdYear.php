...
// SQL query to fetch Third Year data with a "Status" column
$sql = "
  SELECT
    *,
    IF(
...
if ($result === false) {
    echo "Error fetching Third Year data: " . $connection->error;
...
<div class="container"> <!-- Container with rounded corners -->
    ...
    <h2>Third Year Checklist of Courses</h2>
    ...
    <div id="third-year-checklist">
        ...
        <?php else: ?>
            <p><b>No matching third year records found.</p>
        <?php endif; ?>
    </div>
...
function applySearch() {
    ...
    var rows = document.querySelectorAll("#third-year-checklist table tr"); // Get all rows
    ...
    var third = document.getElementById("third-year-checklist");

    if (visibleCount === 0) {
        // If no rows are visible, replace the content with a message
        thirdYearChecklist.innerHTML = "<p><b>No matching third year records found.</b></p>";
    } else {
        ...
        thirdYearChecklist.querySelector("table").innerHTML += summaryRow; // Append the summary row
    }
}
